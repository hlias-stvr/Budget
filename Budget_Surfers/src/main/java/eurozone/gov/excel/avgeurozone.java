...
public class avgeurozone {
    ...
    public static double [] ministrDiv(long[] grministr) {
        ...
        long econ = grministr[12] + (long) (0.80 * grministr[13]) + grministr[8] + grministr[15] + grministr[14] + (long) (0.37 * grministr[9]);
        long envprot = (long) (0.41 * grministr[9]) + (long) (0.22 * grministr[19]);
        long houscomamen = (long) (0.20 * grministr[13]) + (long) (0.22 * grministr[9]);
        ...
        long grsum = publicserv + def + pubordsaf + econ + envprot + houscomamen + health + recrculrel + edu + socialprot;
        long [] sectors = {publicserv, def, pubordsaf, econ, envprot, houscomamen, health, recrculrel, edu, socialprot, grsum};
        double [] grpercent = new double[11];
        for(int i = 0; i < 11; i++) {
            grpercent[i] = Math.round(((double) sectors[i] / GTP * 100)*100.0) / 100.0;
        }
        return grpercent;
    }
    public static double [] compareGrToEurozone(double[] grpercent) {
        ...
        for(int i = 0; i<11; i++) {
            diffgreuz[i] = Math.round((euzpercent[i] - grpercent[i]) * 100.0) /100.0;
        }
        return diffgreuz;
    }
}
