...
public class Subchoices {
    ...
    public void subchoice2a(double[] percSectorsExpenses, String[] grSectors) {
        for (int i = 0; i < 11; i++) {
            System.out.println("Η Ελλάδα δαπανά " + percSectorsExpenses[i]
                + "%" + " στον τομέα " + grSectors[i]);
        }
    }
    ...
    public void subchoice2b(double[] percDiffSectorsExpenses,
        ...
        for (int i = 0; i < 11; i++) {
            if (percDiffSectorsExpenses[i] > 0) {
                ...
                System.out.println("Η Ελλάδα δαπανά το ίδιο ποσοστό στον τομέα "
                    + grSectors[i] + " από τον ΜΟ της Ευρωζώνης");
            }
        }
    }
            ...
            while (true) { // έλεγχος για τις υποεπιλογές
                try {
                    ...
                    if (choice4 < 0 || choice4 > 2) {
                        // έλεγχος για αυτές τις υποεπιλογές
    ...
    public void subchoice4a(double[] euzGdpPerCapita, String[][] gdppop) {
        for (int i = 0; i < euzGdpPerCapita.length; i++) {
            if (i < 19) {
                System.out.println("Τα ΚΚΑΕΠ της χώρας " + gdppop[i][0] +
                    " είναι " + euzGdpPerCapita[i]);
            } else if (i == 19) {
                System.out.println("Τα ΚΚΑΕΠ του ΜΟ της Ευρωζώνης" +
                    " είναι " + euzGdpPerCapita[i]);
            }
        }
    }
    ...
    public void subchoice4b(String[][] gdpPop, double[] euzGdpPerCapita,
        ...
        do {
            while (true) { //μέχρι να δώσει ο χρήστης σωστή τιμή
                try {
                    System.out.println("Δώσε αριθμό για την χώρα που θες" +
                        " να συγκρίνεις με την Ελλάδα ή 0 για πίσω");
                    a = scanner.nextInt();
                    if (a < 0 || a > 20) {
                        throw new IllegalArgumentException(" Ο αριθμός" +
                            " πρέπει να είναι από 1 μέχρι 20");
                    }
                    if (a == 0) {
                        break;
                    }
                    ...
                    System.out.println("Πρόβλημα με τον" +
                        " πίνακα: " + e.getMessage());
                } catch (Exception e) {
                    System.out.println("Πρέπει να δώσεις ακέραιο αριθμό");
                    scanner.nextLine(); //καθάρισμα εισόδου
    ...
    public void subchoice5b(double[] expensesPerRegion, String[][] budget) {
        for (int i = 0; i < expensesPerRegion.length; i++) {
            System.out.println(budget[i + 25][1] +
                 " " + expensesPerRegion[i] + "%");
        }
    }
    ...
    public void subchoice7a(String[][] budget, Scanner scanner) {
        ...
        do {
            ...
            System.out.println("Επίλεξε\n1 για να δείς τις νέες" +
                " ποσοστιάιες δαπάνες της Ελλάδας ανά τομέα");
            System.out.println("2 για να τις συγκρίνεις" +
                " τα νέα ποσοστά με τους τομείς της Ευρωζώνης");
            System.out.println("0 για πίσω");
            while (true) {
                try {
