...
// ================= ΚΥΡΙΟ ΜΕΝΟΥ =================
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            if ("GET".equals(method)) {
                ...
                if ("finish".equals(action)) {
                    ...
                    sendHtml(exchange,
                        resultPageWithTitle("Αποτελέσματα μετά την αποθήκευση"
                        + " αλλαγών", resultsHtml));
                    return;
                }
    ...
    private static void processEdit(Map<String, Object> session,
    ...
    private static String getResultsAfterEdit(String type) {
        ...
        try {
            ...
            if ("sectors".equals(type) && modifiedSectorPercents != null) {
                ...
                double[] newCompare =
                    AvgEurozone.compareGrToEurozone(modifiedSectorPercents);
                String[] sectors = AvgEurozone.sectors();

                /*
                * Εμφανίζουμε μόνο τους 10 τομείς (χωρίς το grsum) στα
                * αποτελέσματα για καθαρότητα
                */
                ...
                for (int i = 0; i < limit; i++) {
                    String text = (newCompare[i] > 0)
                        ? "Λιγότερο κατά <b>" + String.format("%.2f",
                            newCompare[i]) + "%</b>"
                ...
                double[] perPerson =
                    RegionalPer.calcBudgetPerPerson(longAmounts);
                double[] perRegion =
                ...
                try {
                    /*
                    * Διασφάλιση ότι ο πίνακας έχει τουλάχιστον 3 στοιχεία για
                    * να βρούμε τους φόρους (index 1 και 2)
                    * */
                    if (modifiedRevenueAmounts.length >= 3) {
                        // Υπολογισμός Φόρων (π.χ. Άμεσοι + Έμμεσοι Φόροι)
                        long grTaxes = (long) modifiedRevenueAmounts[0] +
                            (long) modifiedRevenueAmounts[1];
    ...
    private static String getEditPage(Map<String, Object> session,
        boolean afterSave) {
        if (afterSave) {
        ...
        /*
        * Ορίζουμε το εσωτερικό μήκος και το μήκος που θα εμφανίζεται στον
        * χρήστη
        */
    ...
    private static String[] getLabels(String type) {
        if ("sectors".equals(type)) {
            return AvgEurozone.sectors();
        }
        if ("regions".equals(type)) {
            return new String[]{"Αττική",
    ...
    private static String resultPage(String result, int main) {
        ...
        String subMenuButtonStyle =
            hasNoSubMenu ? "display:none;" : "display:inline-block;";
        return """
            <!DOCTYPE html>
            <html lang="el">
        ...
        switch (main) {
            case 1 -> {
                ...
                if (sub == 1) {
                    ...
                    for (int i = 0; i < 11; i++) {
                        if (C[i] > 0) {
                            sb.append("Λιγότερο κατά ").append(String.format("%.2f", C[i])).append("% στον ").append(sectors[i]).append("\n");
                        } else if (C[i] < 0) {
                            sb.append("Περισσότερο κατά ").append(String.format("%.2f", Math.abs(C[i]))).append("% στον ").append(sectors[i]).append("\n");
                        } else {
                ...
                if (sub == 1) {
                    ...
                    for (int i = 0; i < g.length; i++) {
                        ...
                        for (int j = 0; j < 4; j++) {
                            sb.append(String.format(" %8.2f%%", g[i][j]));
                        }
                        sb.append("\n");
                    }
                    sb.append("\nΠοσά διαφορές:\n");
                    for (int i = 0; i < n.length; i++) {
                        sb.append(String.format("%-40s", revenue[i][1]));
                        for (int j = 0; j < 4; j++) {
                            sb.append(String.format(" %,12d", n[i][j]));
                        }
                        sb.append("\n");
                    }
                } else if (sub == 2) {
                    ...
                    for (int i = 0; i < h.length; i++) {
                        ...
                        for (int j = 0; j < 4; j++) {
                            sb.append(String.format(" %8.2f%%", h[i][j]));
                        }
                        sb.append("\n");
                    }
                    sb.append("\nΠοσά διαφορές:\n");
                    for (int i = 0; i < m.length; i++) {
                        sb.append(String.format("%-40s", revenue[i+14][1]));
                        for (int j = 0; j < 4; j++) {
                            sb.append(String.format(" %,12d", m[i][j]));
                        }
                        sb.append("\n");
                    }
            ...
            case 4 -> {
                ...
                if (sub == 1) {
                    ...
                    String[] countries = {"Αυστρία", "Βέλγιο", "Κροατία",
                        "Κύπρος", "Εσθονία", "Φινλανδία", "Γαλλία",
                        "Γερμανία", "Ιρλανδία", "Ιταλία", "Λετονία",
                    ...
                    String[] countries = {"Αυστρία", "Βέλγιο", "Κροατία",
                        "Κύπρος", "Εσθονία", "Φινλανδία", "Γαλλία", "Γερμανία",
                        "Ιρλανδία", "Ιταλία", "Λετονία", "Λιθουανία",
    ...
    private static void appendFirstRows(StringBuilder sb, String[][] data,
        int n) {
    ...
    private static Map<String, String> parseParams(String body) {
        ...
        for (String pair : body.split("&")) {
            ...
            if (parts.length == 2) {
                map.put(parts[0], java.net.URLDecoder.decode(parts[1],
                    StandardCharsets.UTF_8));
            }
        }
        return map;
    ...
    private static void sendHtml(HttpExchange e, String html)
        throws IOException {
